...
let EMOJI_CARDS = getFromStorage('emojiCards', DEFAULT_EMOJI_CARDS);

// Initialize cards in localStorage if not exists
if (!localStorage.getItem('emojiCards')) {
    saveToStorage('emojiCards', EMOJI_CARDS);
}
...
function updateAdminStats() {
    ...
    if (resetBadge) {
        resetBadge.textContent = pendingResetCount;
        resetBadge.style.display = pendingResetCount > 0 ? 'inline' : 'none';
    }
...
function resetUserPassword() {
    ...
    // Simpan password plain text (sesuai sistem login)
...
function renderPasswordResetList() {
    ...
    const requests = getFromStorage('passwordResetRequests', []);
    const filtered = requests.filter(r => 
        currentPasswordResetFilter === 'pending' ? r.status === 'pending' : r.status === 'processed'
    ).reverse();

    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align: center;">Tidak ada permintaan ${currentPasswordResetFilter}</td></tr>`;
        return;
    }

    tbody.innerHTML = filtered.map(req => {
        const statusClass = req.status === 'pending' ? 'warning' : 'success';
        const statusText = req.status === 'pending' ? 'Pending' : 'Sudah Diproses';
        ...
        <tr>
            ...
            <td>${req.email}</td>
            <td><span class="status-badge ${statusClass}">${statusText}</span></td>
            <td>${req.newPassword ? `<span class="new-password-display">${req.newPassword}</span>` : '-'}</td>
            <td>
                ${req.status === 'pending' 
                    ? `<button class="btn-action-small btn-edit" onclick="processPasswordReset('${req.id}')" title="Reset & Kirim Email"><i class="fas fa-key"></i></button>`
                    : `<button class="btn-action-small btn-view" onclick="copyPassword('${req.newPassword}')" title="Copy Password"><i class="fas fa-copy"></i></button>`
                }
                <button class="btn-action-small btn-delete" onclick="deletePasswordReset('${req.id}')" title="Hapus"><i class="fas fa-trash"></i></button>
            </td>
        </tr>
    `}).join('');
...
function filterPasswordReset(filter) {
    currentPasswordResetFilter = filter;
    document.querySelectorAll('.password-reset-tabs .tab-btn').forEach(btn => btn.classList.remove('active'));
    event.target.classList.add('active');
    renderPasswordResetList();
}
...
function processPasswordReset(requestId) {
    const requests = getFromStorage('passwordResetRequests', []);
    ...
    user.notifications.push({
        type: 'password_reset',
        message: `Password kamu telah direset oleh admin. Password baru: ${newPassword}`,
        time: new Date().toISOString(),
        read: false
    });
    ...
    saveToStorage('passwordResetRequests', requests);

    // Log
    addAdminLog('password_reset', request.username, 'Password direset oleh admin');

    // Show password to admin
    alert(`Password baru untuk ${request.username}: ${newPassword}\n\nSilakan copy dan kirim ke email: ${request.email}`);
...
function deletePasswordReset(requestId) {
    if (!confirm('Yakin ingin menghapus permintaan ini?')) return;

    const requests = getFromStorage('passwordResetRequests', []);
    const filtered = requests.filter(r => r.id !== requestId);
    saveToStorage('passwordResetRequests', filtered);
...
function renderAdminCards() {
    ...
    grid.innerHTML = EMOJI_CARDS.map(card => `
        <div class="admin-card-item ${card.rarity}">
            <div class="card-actions">
                <button class="btn-card-edit" onclick="event.stopPropagation(); editCard(${card.id})" title="Edit">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn-card-delete" onclick="event.stopPropagation(); deleteCard(${card.id})" title="Delete">
...
function saveCard() {
    ...
    const hp = parseInt(document.getElementById('cardHp').value) || 100;
    const attack = parseInt(document.getElementById('cardAttack').value) || 20;
    const defense = parseInt(document.getElementById('cardDefense').value) || 10;
    const speed = parseInt(document.getElementById('cardSpeed').value) || 10;
    ...
    if (cardId) {
        // Edit existing card
        const cardIndex = EMOJI_CARDS.findIndex(c => c.id === parseInt(cardId));
        if (cardIndex !== -1) {
            EMOJI_CARDS[cardIndex] = {
                ...EMOJI_CARDS[cardIndex],
                emoji,
                name,
                rarity,
            ...
            addAdminLog('edit_card', name, `Updated ${rarity} card`);
            alert('Kartu berhasil diperbarui!');
        }
    } else {
        // Add new card
        const newId = Math.max(...EMOJI_CARDS.map(c => c.id), 0) + 1;
        EMOJI_CARDS.push({
            id: newId,
            emoji,
            name,
        ...
        addAdminLog('add_card', name, `Added new ${rarity} card`);
...
function deleteCard(id) {
    ...
    const cardIndex = EMOJI_CARDS.findIndex(c => c.id === id);
    if (cardIndex !== -1) {
        const cardName = EMOJI_CARDS[cardIndex].name;
        EMOJI_CARDS.splice(cardIndex, 1);
        saveToStorage('emojiCards', EMOJI_CARDS);
        addAdminLog('delete_card', cardName, 'Card deleted');
        renderAdminCards();
...
function saveGachaRates() {
    const common = parseInt(document.getElementById('commonRate').value) || 50;
    const uncommon = parseInt(document.getElementById('uncommonRate').value) || 30;
    const rare = parseInt(document.getElementById('rareRate').value) || 15;
    const epic = parseInt(document.getElementById('epicRate').value) || 4;
    const legendary = parseInt(document.getElementById('legendaryRate').value) || 1;
    
    // Validate total = 100%
    const total = common + uncommon + rare + epic + legendary;
    if (total !== 100) {
        alert(`Total rate harus 100%! Saat ini: ${total}%`);
        return;
    }
    
    const rates = { common, uncommon, rare, epic, legendary };
    saveToStorage('gachaRates', rates);
    addAdminLog('settings', 'gacha_rates', `Updated rates: ${JSON.stringify(rates)}`);
    alert('Rate gacha berhasil disimpan!');
}

function saveStartingResources() {
    const coins = parseInt(document.getElementById('startingCoins').value) || 1000;
    const gems = parseInt(document.getElementById('startingGems').value) || 100;
    
    const resources = { coins, gems };
    saveToStorage('startingResources', resources);
    addAdminLog('settings', 'starting_resources', `Updated: ${JSON.stringify(resources)}`);
    alert('Resource awal berhasil disimpan!');
}

function saveBattleSettings() {
    const winCoins = parseInt(document.getElementById('winCoins').value) || 50;
    const winExp = parseInt(document.getElementById('winExp').value) || 20;
    const loseCoins = parseInt(document.getElementById('loseCoins').value) || 10;
    
    const settings = { winCoins, winExp, loseCoins };
    saveToStorage('battleSettings', settings);
    addAdminLog('settings', 'battle_settings', `Updated: ${JSON.stringify(settings)}`);
...
function loadSettings() {
    // Load gacha rates
    const rates = getFromStorage('gachaRates', { common: 50, uncommon: 30, rare: 15, epic: 4, legendary: 1 });
    document.getElementById('commonRate').value = rates.common;
    document.getElementById('uncommonRate').value = rates.uncommon;
    document.getElementById('rareRate').value = rates.rare;
    document.getElementById('epicRate').value = rates.epic;
    document.getElementById('legendaryRate').value = rates.legendary;
    
    // Load starting resources
    const resources = getFromStorage('startingResources', { coins: 1000, gems: 100 });
    document.getElementById('startingCoins').value = resources.coins;
    document.getElementById('startingGems').value = resources.gems;
    
    // Load battle settings
    const battleSettings = getFromStorage('battleSettings', { winCoins: 50, winExp: 20, loseCoins: 10 });
    document.getElementById('winCoins').value = battleSettings.winCoins;
    document.getElementById('winExp').value = battleSettings.winExp;
    document.getElementById('loseCoins').value = battleSettings.loseCoins;
    
    // Load shop prices
    const shopPrices = getFromStorage('shopPrices', { coins500: 25, coins1000: 45, gems100: 500 });
    document.getElementById('shopCoins500').value = shopPrices.coins500;
    document.getElementById('shopCoins1000').value = shopPrices.coins1000;
    document.getElementById('shopGems100').value = shopPrices.gems100;
    
    // Load topup prices
    const topupPrices = getFromStorage('topupPrices', { 
        coins1000: 10000, coins5000: 45000, 
        gems100: 50000, gems500: 225000, sultan: 500000 
    });
    document.getElementById('topupCoins1000').value = topupPrices.coins1000;
    document.getElementById('topupCoins5000').value = topupPrices.coins5000;
    document.getElementById('topupGems100').value = topupPrices.gems100;
    document.getElementById('topupGems500').value = topupPrices.gems500;
    document.getElementById('topupSultan').value = topupPrices.sultan;
    
    // Load reward settings
    const rewardSettings = getFromStorage('rewardSettings', { 
        starterCoins: 500, starterGems: 50,
        battleWinCoins: 50, battleWinExp: 20, battleLoseCoins: 10
    });
    document.getElementById('starterCoins').value = rewardSettings.starterCoins;
    document.getElementById('starterGems').value = rewardSettings.starterGems;
    document.getElementById('battleWinCoins').value = rewardSettings.battleWinCoins;
    document.getElementById('battleWinExp').value = rewardSettings.battleWinExp;
    document.getElementById('battleLoseCoins').value = rewardSettings.battleLoseCoins;
}
...
function saveShopPrices() {
    const coins500 = parseInt(document.getElementById('shopCoins500').value) || 25;
    const coins1000 = parseInt(document.getElementById('shopCoins1000').value) || 45;
    const gems100 = parseInt(document.getElementById('shopGems100').value) || 500;
    
    const prices = { coins500, coins1000, gems100 };
    saveToStorage('shopPrices', prices);
    addAdminLog('settings', 'shop_prices', `Updated: ${JSON.stringify(prices)}`);
    alert('Harga toko berhasil disimpan!');
    broadcastSync('settings_updated', { type: 'shopPrices' });
}

// ========================================
// TOPUP PRICES SETTINGS
// ========================================

function saveTopupPrices() {
    const coins1000 = parseInt(document.getElementById('topupCoins1000').value) || 10000;
    const coins5000 = parseInt(document.getElementById('topupCoins5000').value) || 45000;
    const gems100 = parseInt(document.getElementById('topupGems100').value) || 50000;
    const gems500 = parseInt(document.getElementById('topupGems500').value) || 225000;
    const sultan = parseInt(document.getElementById('topupSultan').value) || 500000;
    
    const prices = { coins1000, coins5000, gems100, gems500, sultan };
    saveToStorage('topupPrices', prices);
    addAdminLog('settings', 'topup_prices', `Updated: ${JSON.stringify(prices)}`);
    alert('Harga top up berhasil disimpan!');
    broadcastSync('settings_updated', { type: 'topupPrices' });
}
...
function saveRewardSettings() {
    const starterCoins = parseInt(document.getElementById('starterCoins').value) || 500;
    const starterGems = parseInt(document.getElementById('starterGems').value) || 50;
    const battleWinCoins = parseInt(document.getElementById('battleWinCoins').value) || 50;
    const battleWinExp = parseInt(document.getElementById('battleWinExp').value) || 20;
    const battleLoseCoins = parseInt(document.getElementById('battleLoseCoins').value) || 10;
    
    const settings = { starterCoins, starterGems, battleWinCoins, battleWinExp, battleLoseCoins };
    saveToStorage('rewardSettings', settings);
    addAdminLog('settings', 'reward_settings', `Updated: ${JSON.stringify(settings)}`);
    alert('Pengaturan hadiah berhasil disimpan!');
    broadcastSync('settings_updated', { type: 'rewardSettings' });
}
