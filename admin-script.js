 ...
 * Admin Panel Logic with Real-time Online Database
...
async function loadAdminData() {
    ...
    await renderVipStats();
    await renderVipUsersList();
}
...
function startSubscriptions() {
    ...
    topupSubscription = SupaDB.subscribeToTopups(() => {
        renderTopupRequests();
        updateNotificationBadge();
        showNotificationToast('Ada permintaan top up baru!', 'info');
    });
    
    passwordResetSubscription = SupaDB.subscribeToPasswordResets(() => {
        ...
        html += pendingTopups.map(t => `
            <div class="notification-item">
                ...
                <div class="notif-content">
                    ...
                    <span class="notif-time">${formatDate(t.created_at)}</span>
                </div>
            </div>
        `).join('');
    }
        ...
        html += pendingResets.map(r => `
            <div class="notification-item">
                ...
                <div class="notif-content">
                    ...
                    <span class="notif-time">${formatDate(r.created_at)}</span>
                </div>
            </div>
        `).join('');
    }
...
async function updateAdminStats() {
    ...
    document.getElementById('totalCardsAll').textContent = users.reduce((sum, u) => sum + (u.cards?.length || 0), 0);
    document.getElementById('totalBattles').textContent = users.reduce((sum, u) => sum + (u.wins || 0) + (u.losses || 0), 0);
    document.getElementById('totalGacha').textContent = users.reduce((sum, u) => sum + (u.activities?.filter(a => a.icon === 'fa-dice').length || 0), 0);
    document.getElementById('totalTopup').textContent = topups.filter(t => t.status === 'pending').length;
    
    const vipUsers = users.filter(u => (u.vip_level || 0) > 0).length;
    ...
    if (container) {
        container.innerHTML = users.slice(0, 5).map((user, index) => `
            <div class="top-item">
                ...
                <div class="top-info">
                    <span class="top-name">${user.username} ${getVipBadge(user.vip_level)}</span>
...
async function renderOnlinePlayers() {
    ...
    const users = await SupaDB.getAllUsers();
    const userList = users.filter(u => u.status === 'active' || !u.status);
    ...
    container.innerHTML = userList.slice(0, 10).map(user => `
        <div class="online-player-item">
            ...
            <div class="player-info">
                <span class="player-name">${user.username} ${getVipBadge(user.vip_level)}</span>
                <span class="player-stats">${user.wins || 0} menang | ${(user.coins || 0).toLocaleString()} koin</span>
            </div>
...
async function renderUsersTable() {
    ...
    tbody.innerHTML = users.map(user => {
        ...
        <tr>
            ...
            <td>${(user.gems || 0).toLocaleString()}</td>
...
async function editUser(username) {
    ...
    document.getElementById('editUserCreated').textContent = user.created_at ? formatDate(user.created_at) : '-';
    document.getElementById('editUserLastLogin').textContent = user.last_login ? formatDate(user.last_login) : '-';
    ...
    document.getElementById('editUserReason').value = user.status_reason || '';
    document.getElementById('editUserVip').value = user.vip_level || 0;
    document.getElementById('editUserPass').value = user.pass_type || 'none';
...
async function saveUserEdit() {
    ...
    const updates = {
        ...
        status_reason: document.getElementById('editUserReason').value,
        vip_level: parseInt(document.getElementById('editUserVip').value) || 0,
        pass_type: document.getElementById('editUserPass').value,
        email: newEmail
    };
    ...
    if (updates.status !== 'active') {
        await SupaDB.addNotification(username, 'status_changed', `Akun kamu ${updates.status === 'banned' ? 'dibanned' : 'disuspend'}. Alasan: ${updates.status_reason}`);
    }
...
async function renderTopupPackagesAdmin() {
    ...
    const packages = paymentSettings.topup_packages || [];
...
function editPaymentMethod(index) {
    const method = (paymentSettings.payment_methods || [])[index];
    if (!method) return;
...
async function savePaymentMethod() {
    ...
    await SupaDB.updatePaymentSettings({ payment_methods: methods });
...
function editTopupPackage(index) {
    const pkg = (paymentSettings.topup_packages || [])[index];
    if (!pkg) return;
...
async function saveTopupPackage() {
    ...
    const newPackage = {
        id: index === '' ? `pkg_${Date.now()}` : (packages[parseInt(index)]?.id || `pkg_${Date.now()}`),
        name,
        type,
    ...
    await SupaDB.updatePaymentSettings({ topup_packages: packages });
    paymentSettings.topup_packages = packages;
...
async function renderVipStats() {
    ...
    users.forEach(user => {
        ...
        const passType = user.pass_type || 'none';
        passStats[passType]++;
    });
...
async function renderVipUsersList() {
    ...
    const users = await SupaDB.getAllUsers();
    const vipUsers = users.filter(u => (u.vip_level || 0) > 0);
    ...
    container.innerHTML = vipUsers.map(user => `
        <div class="vip-user-item">
            ...
            <div class="vip-user-info">
                ...
                <span class="vip-user-level" style="color: ${getVipColor(user.vip_level)}">${getVipName(user.vip_level)}</span>
            </div>
        </div>
...
async function renderAdminLogs() {
    ...
    container.innerHTML = logs.map(log => `
        <div class="log-item">
            <span class="log-time">${formatDate(log.timestamp || log.created_at)}</span>
