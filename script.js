/**
 * EMOJI BATTLE GACHA - Game Logic (DATABASE VERSION)
 * Professional Gacha Game with Centralized Database
 * Version 4.0 - Real-time Sync, Database Backend
 */
...
window.addEventListener('load', function() {
    ...
    if (savedUser) {
        const user = DB.users.get(savedUser);
        if (user) {
            currentUser = user;
            if (checkUserStatus()) {
                showGameInterface();
    ...
    switch (type) {
        case 'user_updated':
            if (currentUser && data.username === currentUser.username) {
                ...
                currentUser = DB.users.get(currentUser.username);
                updateUserUI();
                updateDashboard();
                updateProfileUI();
            }
        ...
        case 'topup_approved':
            if (currentUser && data.username === currentUser.username) {
                showNotificationToast('Top up kamu telah disetujui!', 'success');
                currentUser = DB.users.get(currentUser.username);
                updateUserUI();
            }
            break;
...
function startSync() {
    // Sync every 3 seconds to keep data fresh
    syncInterval = setInterval(() => {
        if (currentUser) {
            const freshUser = DB.users.get(currentUser.username);
            if (freshUser) {
...
function register() {
    ...
    const success = DB.users.create(username, {
        email,
        password,
        coins: settings.startingResources?.coins || 1000,
        gems: settings.startingResources?.gems || 100
    });
...
function submitForgotPassword() {
    ...
    const reset = DB.passwordReset.create(username, email);
    if (reset) {
...
function addActivity(message, icon = 'fa-info-circle') {
    ...
    DB.users.update(currentUser.username, { activities: currentUser.activities });
}
...
function doGacha(type) {
    ...
    const rates = settings.gachaRates?.[type] || {
    ...
    // Save to database
    DB.users.update(currentUser.username, {
...
function endBattle(playerWon) {
    ...
    const settings = DB.settings.get();
    const rewards = settings.battleRewards || { winCoins: 50, winExp: 20, loseCoins: 10 };
...
function challengePlayer(username) {
    ...
    const opponent = DB.users.get(username);
    if (!opponent) return;
...
function sendChat() {
    ...
    DB.chat.add(message, currentUser.username);
    input.value = '';
    renderChatMessages();
}
...
function startNotificationCheck() {
    updateNotificationBadge();
    notifInterval = setInterval(updateNotificationBadge, 3000);
}
...
function showUserNotifications() {
    ...
    // Mark all as read
    if (unread.length > 0) {
        DB.notifications.markAllAsRead(currentUser.username);
        updateNotificationBadge();
    }
...
function requestTopup(type, amount, price) {
    ...
    document.getElementById('confirmTopupAmount').textContent = amount.toLocaleString();
...
function renderTopupHistory() {
    ...
    const topups = DB.topup.getByUser(currentUser.username);
    ...
    list.innerHTML = topups.slice().reverse().map(t => `
...
function buyItem(type, amount, price) {
    ...
    if (type === 'coins') {
        ...
        DB.users.update(currentUser.username, { coins: currentUser.coins, gems: currentUser.gems });
...
function claimStarterPack() {
    ...
    const settings = DB.settings.get();
    const pack = settings.starterPack || { coins: 500, gems: 50 };
