...
function renderProfileSection() {
    ...
    // Update stats
    document.getElementById('profileCoinsLarge').textContent = currentUser.coins || 0;
    document.getElementById('profileGemsLarge').textContent = currentUser.gems || 0;
    ...
    if (currentUser.vip && currentUser.vip > 0) {
        vipBadge.textContent = getVipName(currentUser.vip);
        vipBadge.style.display = 'inline-flex';
        vipBadge.style.background = getVipColor(currentUser.vip);
    } else {
    ...
    if (currentUser.pass && currentUser.pass !== 'none') {
        passBadge.textContent = getPassName(currentUser.pass);
...
function getVipName(level) {
    const names = { 1: 'VIP Bronze', 2: 'VIP Silver', 3: 'VIP Gold', 4: 'VIP Platinum', 5: 'VIP Diamond' };
    return names[level] || 'VIP';
}

function getVipColor(level) {
    const colors = { 1: '#22c55e', 2: '#3b82f6', 3: '#a855f7', 4: '#f59e0b', 5: '#ef4444' };
    return colors[level] || '#6366f1';
}

function getPassName(pass) {
    const names = { weekly: 'Weekly Pass', monthly: 'Monthly Pass', seasonal: 'Season Pass' };
        ...
        modal.innerHTML = `
            <div class="modal-content profile-edit">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="editProfileUsername" placeholder="Username...">
                </div>
                <div class="form-group">
                    <label>Email</label>
                    <input type="email" id="editProfileEmail" placeholder="Email...">
                </div>
...
function saveProfileEdit() {
    ...
    if (newUsername !== oldUsername) {
        ...
        users[newUsername] = { ...users[oldUsername], username: newUsername, email: newEmail };
        delete users[oldUsername];
        currentUser.username = newUsername;
    } else {
        users[oldUsername].email = newEmail;
    }
    ...
    // Refresh profile section if active
...
function refreshGame() {
    ...
    renderOnlinePlayers();
    renderTopupHistory();
    alert('Game berhasil di-refresh!');
}
