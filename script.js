...
function checkUserStatus() {
    ...
    if (status !== 'active') {
        ...
        document.getElementById('suspendedReason').textContent = 
            status === 'banned' 
                ? `Akun kamu telah dibanned. Alasan: ${currentUser.status_reason || 'Melanggar aturan'}`
                : `Akun kamu telah disuspend. Alasan: ${currentUser.status_reason || 'Pelanggaran ringan'}`;
        return false;
    }
    return true;
}
...
function startChatSubscription() {
    if (!SupaDB.isConnected()) return;
    chatSubscription = SupaDB.subscribeToChat(() => renderChatMessages());
}
...
async function register() {
    ...
    const existingEmail = await SupaDB.getUserByEmail(email);
    if (existingEmail) {
...
function updateUserUI() {
    ...
    document.getElementById('userGems').textContent = (currentUser.gems || 0).toLocaleString();
...
function updateDashboard() {
    ...
    document.getElementById('winStreak').textContent = currentUser.win_streak || 0;
    
    document.getElementById('profileUsername').textContent = currentUser.username;
    document.getElementById('profileEmail').textContent = currentUser.email;
    document.getElementById('profileCoins').textContent = (currentUser.coins || 0).toLocaleString();
    document.getElementById('profileGems').textContent = (currentUser.gems || 0).toLocaleString();
...
function updateProfileUI() {
    ...
    document.getElementById('profileStreakLarge').textContent = currentUser.win_streak || 0;
    document.getElementById('profilePvpWinsLarge').textContent = currentUser.pvp_wins || 0;
...
async function doGacha(type) {
    ...
    const rates = gameSettings?.gacha_rates?.[type] || {
...
async function endBattle(playerWon) {
    ...
    const rewards = gameSettings?.battle_rewards || { winCoins: 50, winExp: 20, loseCoins: 10 };
    ...
    if (playerWon) {
        ...
        currentUser.win_streak = (currentUser.win_streak || 0) + 1;
    ...
    await SupaDB.updateUser(currentUser.username, {
        ...
        win_streak: currentUser.win_streak,
        cards: currentUser.cards
    });
    ...
    onlineList.innerHTML = otherUsers.map(u => `
        <div class="online-player-item">
            ...
            <div class="player-info">
                ...
                <span class="player-stats">${u.pvp_wins || 0} menang PvP</span>
            </div>
            <button class="btn-challenge" onclick="challengePlayer('${u.username}')">Tantang</button>
        </div>
    `).join('');
    
    document.getElementById('pvpWins').textContent = currentUser?.pvp_wins || 0;
    document.getElementById('pvpPoints').textContent = currentUser?.pvp_points || 0;
    
    const allUsers = users.filter(u => u.status === 'active' || !u.status)
        .sort((a, b) => (b.pvp_points || 0) - (a.pvp_points || 0));
...
async function findMatch() {
    ...
    const users = await SupaDB.getAllUsers();
    const opponents = users.filter(u => 
        u.username !== currentUser.username && 
        (u.cards || []).length > 0 && 
        (u.status === 'active' || !u.status)
    );
...
async function endPvP(playerWon) {
    ...
    if (playerWon) {
        ...
        currentUser.pvp_points = (currentUser.pvp_points || 0) + 10;
        ...
        currentUser.pvp_points = (currentUser.pvp_points || 0) + 2;
    ...
    await SupaDB.updateUser(currentUser.username, {
        coins: currentUser.coins,
        pvp_wins: currentUser.pvp_wins,
        pvp_points: currentUser.pvp_points
    });
...
async function sendChat() {
    ...
    await SupaDB.sendChat(currentUser.username, message);
    input.value = '';
    renderChatMessages();
}
...
async function showUserNotifications() {
    ...
    await SupaDB.updateUser(currentUser.username, { notifications: updatedNotifications });
...
function requestTopup(type, amount, price) {
    ...
    const paymentMethod = (paymentSettings?.payment_methods || [])[0];
...
async function claimStarterPack() {
    ...
    if (currentUser.starter_pack_claimed) {
        alert('Kamu sudah mengklaim paket pemula!');
        return;
    }
    
    const pack = gameSettings?.starter_pack || { coins: 500, gems: 50 };
    
    currentUser.coins += pack.coins;
    currentUser.gems += pack.gems;
    currentUser.starter_pack_claimed = true;
